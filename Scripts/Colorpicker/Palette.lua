...
function Palette:Constructor(parent, dimensionX, dimensionY, color)
    ...
    self:SetBackground("WeaverVenomDisplay/Images/ColorPicker/V_x_H.tga");
    self:SetStretchMode(2);
    self.width, self.height = self:GetSize();
    
    self.overlay = Turbine.UI.Window();
    self.overlay:SetBackground("WeaverVenomDisplay/Images/ColorPicker/V_x_H.tga");
    ...
    self.spot:SetBackground("WeaverVenomDisplay/Images/ColorPicker/spot.tga");
...
function Palette:SetGamut(color)
    local sliderDimension = WeaverVenomDisplay.Utils.Color.GetOtherDimensions(self.dimensionY);
    local sliderValue = color:Get(sliderDimension);

    if (string.find("RGB", sliderDimension)) then
        local backColor = WeaverVenomDisplay.Utils.Color(sliderValue, 0, 0, 0);
        backColor[sliderDimension] = 1;
        self.overlay:SetBackColor(backColor);
    elseif (sliderDimension == "S") then
        local backColor = WeaverVenomDisplay.Utils.Color(1 - sliderValue, 1, 1, 1);
        self.overlay:SetBackColor(backColor);
    elseif (sliderDimension == "H") then
        local backColor = WeaverVenomDisplay.Utils.Color(1, 0, 0, 0);
        backColor:SetHSV(sliderValue, 1, 1);
        self:SetBackColor(backColor);
    else
        local backColor = WeaverVenomDisplay.Utils.Color(1 - sliderValue, 0, 0, 0);
        self.overlay:SetBackColor(backColor);
    end
end
...
function Palette:SetDimensions(dimensionX, dimensionY)
    ...
    self.overlay:SetBackground("WeaverVenomDisplay/Images/ColorPicker/" .. dimensionX .. "_x_" .. dimensionY .. ".tga");
    local sliderDimension = WeaverVenomDisplay.Utils.Color.GetOtherDimensions(dimensionY);
    ...
    elseif (sliderDimension == "H") then
        self:SetBackground("WeaverVenomDisplay/Images/ColorPicker/S_x_V_back.tga");
