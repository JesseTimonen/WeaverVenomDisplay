...
function Slider:Constructor(parent, dimension, color)
    ...
    self:SetBackground("WeaverVenomDisplay/Images/ColorPicker/B_grad.tga");
    self:SetStretchMode(2);
    self.width, self.height = self:GetSize();

    self.overlay = Turbine.UI.Window();
    self.overlay:SetBackground("WeaverVenomDisplay/Images/ColorPicker/B_grad.tga");
...
function Slider:SetGamut(color)
    ...
    if (string.find("RGB", self.dimension)) then
        local backColor = WeaverVenomDisplay.Utils.Color(1, color.R, color.G, color.B);
        backColor[self.dimension] = 0;
        self.overlay:SetBackColor(backColor);
    elseif (self.dimension == "H") then
        local H, S, V = color:GetHSV();
        self:SetBackColor(WeaverVenomDisplay.Utils.Color(1 - S, 1, 1, 1));
        self.overlay:SetBackColor(WeaverVenomDisplay.Utils.Color(1 - V, 0, 0, 0));
...
function Slider:SetDimension(dimension)
    ...
    self.overlay:SetBackground("WeaverVenomDisplay/Images/ColorPicker/" .. dimension .. "_grad.tga");

    if (string.find("RGB", self.dimension)) then
        self.overlay:SetBackColorBlendMode(Turbine.UI.BlendMode.Screen);
    elseif (self.dimension == "H") then
        self:SetBackground("WeaverVenomDisplay/Images/ColorPicker/H_grad.tga");
    ...
    elseif (self.dimension == "V") then
        ...
        self:SetBackground("WeaverVenomDisplay/Images/ColorPicker/S_grad.tga");
